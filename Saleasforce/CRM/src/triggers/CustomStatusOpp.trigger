trigger CustomStatusOpp on Opportunity (after update) {
...
List<Database.DeleteResult> result = Database.delete(opportunityLineItemList);
            System.debug(opportunityLineItemList.size());
           for(Database.DeleteResult d : result){
               System.debug(d.isSuccess());
           }
}
}
}