 ...
 * 
 * jquery.impulse-slider.js is a jQuery 3D slider plugin based on the power of
 ...
 * @pscnunes
 * 
 * version: 0.4 url: http://www.twoimpulse.com/products/impulse-slider
 * 
 * Copyright 2013 TwoImpulse Free to use under the MIT license.
 * 
 */
...
(function($) {
	...
	$.fn.impulseslider = function(options) {
		...
		var rightSelector = getString(opts.rightSelector,
				defaults.rightSelector);
		var leftSelector = getString(opts.leftSelector, defaults.leftSelector);
		var pauseSelector = getString(opts.pauseSelector,
				defaults.pauseSelector);
		directionRight = getBoolean(opts.directionRight,
				defaults.directionRight);
		height = toNumber(opts.height, defaults.height);
		width = toNumber(opts.width, defaults.width);
		spinnerSelector = getString(opts.spinnerSelector,
				defaults.spinnerSelector);
		containerSelector = getString(opts.containerSelector,
				defaults.containerSelector);
		var autostart = getBoolean(opts.autostart, defaults.autostart);
		paused = !autostart;
		pauseTime = toNumber(opts.pauseTime, defaults.pauseTime);
		var transitionDuration = toNumber(opts.transitionDuration,
				defaults.transitionDuration);
		var transitionEffect = getString(opts.transitionEffect,
				defaults.transitionEffect);
		var images = toStringArray(opts.images, defaults.images);
		var imageDivClasses = toStringArray(opts.imageDivClasses,
				defaults.imageDivClasses);
		degreesRotation = toNumber(opts.degreesRotation,
				defaults.degreesRotation);
		...
		$(rightSelector).click(
				function() {
					...
					console.log("Manually rotated %n degrees to the right",
							degreesRotation);
					paused = false;
				});
		...
		$(leftSelector).click(
				function() {
					...
					console.log("Manually rotated %n degrees to the left",
							degreesRotation);
					paused = false;
				});
			...
			var infiniteLoop = setInterval(
					function() {
						if (!paused) {
							if (isActive) {
								var now = new Date();
								var rotatedLongAgo = dateDiffGreaterThan(now,
										lastRotated, pauseTime / 2);
								if (rotatedLongAgo) {
									rotate(spinnerSelector, degreesRotation);
									console.log("Auto rotated %n degrees",
											degreesRotation);
								} else {
									console
											.log(
													"Cancelled auto rotate - last rotation: %s millisecs ago",
													now - lastRotated);
								}
							} else {
		...
		// TODO split the transition CSS3 properties to avoid this string
		// concatenation
	...
	$.fn.impulseslider.defaults = {
		...
		transitionEffect : "ease", // "linear", "ease","ease-in","ease-out",
									// "ease-in-out" or
		autostart : true,
		rightSelector : ".right",
		leftSelector : ".left",
	...
	$.fn.impulseslider.rotateRight = function() {
		var now = new Date();
		var rotatedLongAgo = dateDiffGreaterThan(now, lastRotated,
				pauseTime / 2);
		if (rotatedLongAgo) {
			rotateRight(spinnerSelector, degreesRotation);
		}
	...
	var buildPrisma = function(containerSelector, height, width, depth,
			perspective, divs) {
	...
	var transformObj = function(axis, deg, obj, depth) {
		var rotation = "rotate" + axis + "(" + deg + "deg) " + 'translateZ('
				+ depth + 'px)';
